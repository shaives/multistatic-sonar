...
DIR = "Instances/StraitOfJuanDeFuca/" # directory where to find input and store output files

INPUT = "GMRTv3_4_20171029topo.asc" # file name of ocean floor data
OUTPUT = "StraitOfJuanDeFuca" # name of output file (a time stamp will be added in front of this)

X = 12 # number of pixels in x-direction
Y = 11 # number of pixels in y-direction

GOAL = 1 # optimization goal: cover all pixels, minimize cost for deployed equipment (0) or deploy equipment, maximize coverage (1)

S = 1 # EITHER: cost for each deployed source (if GOAL=0), OR: number of deployed sources (if GOAL=1)
R = 3 # EITHER: cost for each deployed receiver (if GOAL=0), OR: number of deployed receivers (if GOAL=1)
